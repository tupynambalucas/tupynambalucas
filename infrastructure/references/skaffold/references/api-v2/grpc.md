...
This is a generated reference for the [Skaffold API](../../design/api.md) gRPC layer.

We also generate the [reference doc for the HTTP layer](../api/swagger.md).

<a name="v2/skaffold.proto"></a>

## v2/skaffold.proto

You can find the source for v2/skaffold.proto
[on Github](https://github.com/GoogleContainerTools/skaffold/blob/main/proto/v2/skaffold.proto).
...
`ApplicationLogEvent` represents a log that comes from one of the pods running in the user's
application.
...
`BuildSubtaskvent` describes the build status per artifact, and will be emitted by Skaffold anytime
a build starts or finishes, successfully or not. If the build fails, an error will be attached to
the event.
...
A Resource StatusCheck Event for a Cloud Run Service. Indicates that a Cloud Run Service has
deployed successfully and is serving on the specified URL
...
`DeploySubtaskEvent` represents the status of a deployment, and is emitted by Skaffold anytime a
deployment starts or completes, successfully or not.
...
`Event` describes an event in the Skaffold process. It is one of MetaEvent, BuildEvent, TestEvent,
DeployEvent, PortEvent, StatusCheckEvent, ResourceStatusCheckEvent, FileSyncEvent, or
DebuggingContainerEvent.
...
`ExecSubtaskEvent` represents the status of an exec, and is emitted by Skaffold anytime an exec
starts or completes, successfully or not.
...
`RenderSubtaskEvent` represents the status of a render, and is emitted by Skaffold anytime a render
starts or completes, successfully or not.
...
`SkaffoldLogEvent` represents a piece of output that comes from a skaffold run, for example:
"Generating tags...", "Step 1/3 : FROM gcr.io/distroless/base"
...
log level | | message | [string](#string) | | contents of the log |
...
A Resource StatusCheck Event, indicates progress for each kubernetes deployment. For every resource,
there will be exactly one event with `status` _Succeeded_ or _Failed_ event. There can be multiple
events with `status` _Pending_. Skaffold polls for resource status every 0.5 second. If the resource
status changes, an event with `status` “Pending”, “Complete” and “Failed” will be sent with the new
status.
...
`TaskEvent` represent the different larger phases of a skaffold session, for example Build, Deploy,
etc. If a phase fails, an actionable error will be attached
...
`TestSubtaskEvent` represents the status of a test, and is emitted by Skaffold anytime a test starts
or completes, successfully or not.
...
`VerifyEvent` represents the status of a render, and is emitted by Skaffold anytime a render starts
or completes, successfully or not.
...
Enum for Status codes<br> These error codes are prepended by Phase Name e.g. INIT, BUILD, TEST,
DEPLOY, STATUSCHECK, DEVINIT<br> For Success Error codes, use range 200 to 250.<br> For Unknown
error codes, use range 500 to 600.<br> For Cancelled Error code, use range 800 to 850.<br>
