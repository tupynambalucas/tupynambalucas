...
We also generate the [reference doc for the HTTP layer](swagger.md).

<a name="v1/skaffold.proto"></a>

## v1/skaffold.proto

You can find the source for v1/skaffold.proto
[on Github](https://github.com/GoogleContainerTools/skaffold/blob/main/proto/v1/skaffold.proto).
...
`BuildEvent` describes the build status per artifact, and will be emitted by Skaffold anytime a
build starts or finishes, successfully or not. If the build fails, an error will be attached to the
event.
...
`DeployEvent` represents the status of a deployment, and is emitted by Skaffold anytime a deployment
...
`StatusCheckEvent` describes if the status check for kubernetes rollout has started, is in progress,
...
`TestEvent` represents the status of a test, and is emitted by Skaffold anytime a test starts or
