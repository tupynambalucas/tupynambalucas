...
- `user`: explicit port-forwards defined in the `skaffold.yaml` (called
  [_user-defined port forwards_](#UDPF))
...
These defaults can be overridden with the `--port-forward` flag, and port-forwarding can be disabled
with `--port-forward=off`.
...
> [!NOTE] Note that `skaffold debug --port-forward` previously enabled the equivalent of `pods` as
> Skaffold did not have an equivalent of `debug`. We have replaced `pods` as it caused confusion.

### User-Defined Port Forwarding {#UDPF}

Users can define additional resources to port forward in the skaffold config, to enable port
forwarding for
...
For this example, Skaffold will attempt to forward port 8080 to `localhost:9000`. If port 9000 is
unavailable, Skaffold will forward to a random open port.

> [!NOTE] Skaffold will request matching local ports only when the remote port is `> 1023`. So a
> service on port `8080` would still map to port `8080` (if available), but a service on port `80`
> will be mapped to some port `≥ 1024`.
>
> User-defined port-forwards in the `skaffold.yaml` are unaffected and can bind to system ports.

> [!NOTE] When [deploying to Docker](deployers/docker.md) with a user-defined port-forward in the
> `skaffold.yaml`, the `resourceType` of `portForward` must be set to `container`. Otherwise,
> Skaffold will not tell the Docker daemon to expose that port.

Skaffold will run `kubectl port-forward` on each of these resources in addition to the automatic
port forwarding described above. Acceptable resource types include: `Service`, `Pod` and Controller
resource type that has a pod spec: `ReplicaSet`, `ReplicationController`, `Deployment`,
...
Skaffold will run `kubectl port-forward` on all user defined resources. `kubectl port-forward` will
select one pod created by that resource to forward too.
...
If you want the port forward to to be available from other hosts and not from the local host only,
you can bind the port forward to the address `0.0.0.0`:
