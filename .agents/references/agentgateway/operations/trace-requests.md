...
- Install the [agentgateway binary](../quickstart/index.md) and have a `config.yaml` ready. The examples in this guide use the [non-agentic HTTP quickstart](../quickstart/non-agentic-http.md) setup, with agentgateway listening on `:3000` and forwarding to httpbin on `:8000`.
...
The host portion of the URL (`example.com`) sets the `Host` header on the request but is not used
...
[Debug your setup](/docs/standalone/latest/operations/debug/ 'Debug your setup')[Inspect agentgateway configuration](/docs/standalone/latest/operations/inspect-config/ 'Inspect agentgateway configuration')
