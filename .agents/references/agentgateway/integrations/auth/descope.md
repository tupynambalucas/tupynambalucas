...
token in an [authorization](../../configuration/security/mcp-authz.md) policy.
...
1. Add an `authorization` policy alongside `mcpAuthentication` in your `config.yaml` that requires the
   ...
   > [!NOTE] Note Where the roles claim appears depends on your Authorization Claims Configuration . With the default Descope JWT, roles are in jwt.tenants["<YOUR TENANT ID>"].roles . With the No Tenant Reference claim format, roles are in jwt.roles , which is what this rule uses.
...
[Tailscale](/docs/standalone/latest/integrations/auth/tailscale/ 'Tailscale')[Okta](/docs/standalone/latest/integrations/auth/okta/ 'Okta')
