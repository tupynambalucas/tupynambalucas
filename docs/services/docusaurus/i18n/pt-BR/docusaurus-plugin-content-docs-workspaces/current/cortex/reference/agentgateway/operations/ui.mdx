...
integrations](/workspaces/cortex/reference/agentgateway/integrations/web-uis), which are third-party AI chat
...
2. Open [http://localhost:15000/ui/](http://localhost:15000/ui/) in your browser.
   ...
   ![](https://agentgateway.dev/img/agentgateway-ui-landing.png)

   ![](https://agentgateway.dev/img/agentgateway-ui-landing-dark.png)
...
1. Configure at least one LLM model and, if the gateway requires client authentication, a [virtual API key](/workspaces/cortex/reference/agentgateway/llm/cost-controls/virtual-keys).
...
prerequisites, see [LLM clients](/workspaces/cortex/reference/agentgateway/integrations/llm-clients).
...
[OIDC](/workspaces/cortex/reference/agentgateway/configuration/security/oidc) policy. When you attach the UI to a
...
2. Add a `ui` section to your config file that attaches to a gateway and applies an `oidc` policy. The
   ...
   to users whose email address ends in `@%PROJECT_DOMAIN%`.
   ...
   ui:
     policies:
       ...
       authorization:
         rules:
         - allow: jwt.email.endsWith("@%PROJECT_DOMAIN%")
   ```
...
4. Open the UI at the gateway’s address, such as [http://localhost:3000/ui/](http://localhost:3000/ui/). Instead of loading the UI
...
authentication](/workspaces/cortex/reference/agentgateway/configuration/security/oidc) and the
