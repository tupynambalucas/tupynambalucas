...
authentication, see [MCP configuration modes](/workspaces/cortex/reference/agentgateway/mcp/configuration-modes).
...
routing](/workspaces/cortex/reference/agentgateway/configuration/traffic-management/matching#backend-routing).
...
2. Review the configuration file.
   ...
   [config.yaml](/workspaces/cortex/reference/agentgateway/./mcp/connect/https:/agentgateway.dev/examples/mcp-multiplex/config.yaml)
...
5. Verify that the **Result** panel reports the discovered tools and that each tool name is prefixed
   ...
   ![](https://agentgateway.dev/img/ui-playground-multi-tools-dark.png)
...
7. Verify access to a tool from the `time` target.
   ...
   3. Verify that the **Result** panel returns `HTTP 200` with the current time in the **Tool output**.
      ...
      ![](https://agentgateway.dev/img/ui-tool-time-current-dark.png)
...
Apps](/workspaces/cortex/reference/agentgateway/mcp/apps) that issue tool calls from a rendered UI. Because unprefixed
...
- Apply different policies to different MCP targets with [MCP target policies](/workspaces/cortex/reference/agentgateway/mcp/mcp-target-policies).
