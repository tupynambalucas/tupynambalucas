---
title: 'Model costs'
description: 'Price LLM requests with a model cost catalog and expose realized USD costs in logs, traces, metrics,'
sidebar_position: 2
---
...
For general LLM telemetry setup, see [Observe traffic](/workspaces/cortex/reference/agentgateway/llm/observability).
...
import`](/workspaces/cortex/reference/agentgateway/reference/agctl/agctl-costs-import) reference.

## Import costs (Admin UI)

You can also manage the model cost catalog from the built-in [Admin
UI](/workspaces/cortex/reference/agentgateway/operations/ui).
...
1. Open the [Admin UI cost page](http://localhost:15000/ui/llm/costs) (**LLM > Costs**). The page lists
   ...
   ![Admin UI LLM Costs page showing catalog sources and custom cost overrides](https://agentgateway.dev/img/ui-cost-catalog.png)

   ![Admin UI LLM Costs page showing catalog sources and custom cost overrides](https://agentgateway.dev/img/ui-cost-catalog-dark.png)
...
see [Cost dashboard](/workspaces/cortex/reference/agentgateway/llm/cost-controls/dashboard).
...
- Use [Rate limiting](/workspaces/cortex/reference/agentgateway/configuration/resiliency/rate-limits) to cap request or token usage per route, user, or API key.
- Use [Virtual keys](/workspaces/cortex/reference/agentgateway/llm/cost-controls/virtual-keys) to issue keys with per-key controls and attribution.
