...
Um Contexto Delimitado define a fronteira explícita dentro da qual um modelo de domínio é válido. Dentro desta fronteira, nossa Linguagem Ubíqua (Ubiquitous Language) possui significados precisos e indiscutíveis. No `%PROJECT_DOMAIN%`:
...
O padrão de **Shared Kernel** é utilizado quando os contextos compartilham uma parte estável de esquemas de domínio ou variáveis de design. Em nosso sistema, o pacote **`@repo/studio/assets`** atua como nosso Shared Kernel. Tanto o `@repo/hub/web` quanto o `@/renderer` importam os tokens de estilo CSS e ícones SVG diretamente deste pacote.
...
- **Limites Horizontais**: Camadas arquiteturais internas. Por exemplo, o aplicativo web React `@repo/hub/web` (camada de UI/Feature) é totalmente desacoplado do pacote `@repo/hub/core` (camada de domínio/contratos), forçando o fluxo de dependências estritamente para baixo.
...
Quando um Contexto Delimitado (Bounded Context) inclui unidades de implantação em contêineres (gerenciadas localmente via Podman Compose ou manifestos Kubernetes com Skaffold/Minikube), o layout dos diretórios deve ser estruturado para corresponder ao seu alinhamento arquitetural:
...
- **Quando Usar**: Aplicado a workspaces focados em produtos específicos (como [/hub](/docs/explanation/domain-driven-monorepo#31-topologia-de-pastas) e `studio/design`) que representam pilhas tradicionais de aplicações web (Frontend Client, Backend API, Banco de Dados, Cache).
