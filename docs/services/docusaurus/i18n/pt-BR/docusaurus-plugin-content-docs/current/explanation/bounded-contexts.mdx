...
description: Explicação detalhada de Contextos Delimitados (Bounded Contexts), estrutura do monorepo e papéis dos workspaces no %PROJECT_DOMAIN%.
---
...
graph TD
    direction TB
    Root["%PROJECT_DOMAIN% Monorepo"]
    Root --> Hub["Hub Context: /hub"]
    Root --> Cortex["Cortex Context: /cortex"]
    Root --> Renderer["Renderer Context: /renderer"]
    ...
    Renderer --> R_Gen["TypeScript Generator"]
...
| :------------------ | :-------------------- | :----------------- | :------------------------------------------------------------------ |
| `hub/services/web`  | `@repo/hub/web`       | **Aplicação**      | Cliente React do portfólio pessoal, blog e painel de administração. |
| `hub/services/api`  | `@repo/hub/api`       | **Aplicação**      | Backend Fastify REST API servindo posts de blog e formulários.      |
| `hub/packages/core` | `@repo/hub/core`      | **Biblioteca**     | SSOT para o contexto Hub (validação Zod, esquemas compartilhados).  |
| `cortex/`           | `@/cortex`            | **Ferramental**    | Gateway unificado de IA, adaptadores MCP e executores de agentes.   |
| `renderer/`         | `@/renderer`          | **Aplicação**      | Aplicação CLI em Zig compilando métricas do GitHub e gerando SVGs.  |
| `studio/assets`     | `@repo/studio/assets` | **Biblioteca**     | Tokens de design, logos, SVGs e utilitários de sincronização.       |
| `tools/`            | `@repo/tools/*`       | **Ferramental**    | Controle de versão Git e automação do GitHub CLI isolados.          |
| `platform/`         | `@/platform`          | **Infraestrutura** | Monitoramento de telemetria e servidores de cache Turborepo.        |
| `docs/`             | `@/docs`              | **Docs Hub**       | Portal do desenvolvedor Docusaurus autoritativo.                    |

---

## Filosofia de Contextos Delimitados

- **Isolamento de Contexto**: Cada diretório raiz (`hub/`, `cortex/`, `renderer/`, `studio/`, `tools/`, `platform/`, `docs/`) representa um contexto isolado. Tipos, contratos e configurações são encapsulados localmente, referenciando bibliotecas externas apenas por fronteiras de pacotes.
- **Desacoplamento Estrito**: As regras de negócio do Developer Hub (`hub/`) são completamente desacopladas do compilador de estatísticas do perfil (`renderer/`) e do hub de IA (`cortex/`).
