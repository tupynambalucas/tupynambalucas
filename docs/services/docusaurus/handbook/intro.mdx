...
The monorepo coordinates the developer's personal website (`hub`), automated stats visualizations (`renderer`), design systems (`studio`), AI processing hub (`cortex`), and automation tools (`tools`).

The primary customer-facing application is the **Developer Hub** (`@repo/hub/*`):
...
1. **Portfolio Presentation**: Visitors explore the frontend `@repo/hub/web`, consuming blog posts and project highlights served by `@repo/hub/api`.
2. **Contact Submission**: Form inputs are validated via Turnstile anti-spam keys, serialized using Zod schemas from `@repo/hub/core`, and persisted to a MongoDB instance.
3. **Renderer Statistics Generation**: The `@/renderer` workspace compiles GitHub metrics using a native Zig CLI and dynamically overwrites the root `README.md` file.
...
- **`@repo/hub/*`**: Personal React/Fastify hub.
- **`@/renderer`**: Zig-based GitHub profile stats compiler.

---

## 6. Quick Start

Ensure you have **Node.js 22+**, **PNPM 11+**, **Zig**, **Podman** (with Compose extension), **Minikube**, and **Skaffold** installed.
...
3. **Start Local Infrastructure & Applications:**
   We provide unified orchestration scripts via Turborepo and Skaffold. You can run in Podman Compose mode (`pnpm cortex:up` and `pnpm hub:dev`) or Kubernetes mode (`pnpm minikube:up` and `pnpm k8s:dev`).
   See [MONOREPO.readme.md](https://github.com/workspace/workspace/blob/develop/MONOREPO.readme.md) or the [Command Reference](./reference/commands.mdx) for details.
