---
title: Overview
description: Comprehensive overview of %PROJECT_DOMAIN%'s monorepo structure, bounded contexts, and technology stack.
sidebar_position: 1
---
...
%PROJECT_DOMAIN% is a personal developer portal, portfolio, blog, and automation engine. The system is built on a **Monorepo** architecture using **PNPM Workspaces** and **Turborepo**, prioritizing high performance, strict typing, and domain isolation through a **Bounded Context** strategy.

The architecture is designed to host the developer hub website (`hub`) and the automated stats compiler (`renderer`), logically decoupled at the root level.

---

## 2. Monorepo Structure & Bounded Contexts

The %PROJECT_DOMAIN% codebase is organized into **Domain Contexts** at the root level. For a detailed breakdown of the application roles, context philosophy, and directory structures, please refer to the Bounded Context guide:
...
- **Infrastructure Coupling:** Orchestrating backing services as pre-requisites for application development (e.g., `pnpm hub:up` or `pnpm cortex:up` starts databases/backing services before the API).
...
To ensure extreme maintainability, long-term readability, and clean boundaries across our domain contexts, %PROJECT_DOMAIN% follows four foundational architectural principles.
...
Designed for **Self-Hosted Excellence**, our Kubernetes-native orchestration, Skaffold development pipelines, environment variables strategy, and networking standards are managed centrally in the Infrastructure workspace.

For a comprehensive, deep-dive guide on directory structures, Minikube deployment, Cloudflare tunnel routing, and Kustomize overlays, explore the dedicated Infrastructure workspace documentation:
  ...
  items={[
    {
      ...
      description:
        'Explore the centralized Kubernetes orchestration, GitOps deployment pipelines, and edge-to-cluster networking topology.',
    },
    {
      type: 'link',
      ...
      description:
        'Learn how to run the monorepo locally using Minikube and Skaffold for live hot-reloading.',
    },
  ]}
/>
...
- **AI Contextual Bridge & Ingress**: Managed in the **[Cortex Workspace](/workspaces/cortex)**.

---

## 10. Security Standards

%PROJECT_DOMAIN% follows a multi-layered security strategy. For detailed implementation details, refer to the **[Security Architecture](/docs/reference/security)** document.
