...
The `%PROJECT_DOMAIN%` project relies on Cloudflare R2 Object Storage (an S3-compatible, zero-egress fee object storage service) to manage binary assets, high-fidelity design projects, and self-hosted editor persistence.
...
| :----------------------- | :----------------- | :------------------------------------------------------------------------------------------------------------------------------------- | :----------------------------------------- |
| **`workspace-assets`**   | Public (CDN)       | Hosts web-ready assets (optimized images, SVGs, GLB models, EXR files). Served publicly to frontends like `downstream consumers`.      | `studio/bucket/.env.studio.bucket`         |
| **`workspace-creative`** | Private            | Vault for raw master files (Blender `.blend`, Illustrator `.ai`, Photoshop `.psd`, Premiere `.prproj`). Restricted to core developers. | `studio/bucket/.env.studio.bucket`         |
| **`workspace-penpot`**   | Private (Internal) | Holds user uploads, layouts, custom vector libraries, and document backups for the self-hosted Penpot design tool.                     | `studio/design/infrastructure/docker/.env` |

---

## 2. Asset Synchronizer CLI (`@repo/studio/bucket`)

The `@repo/studio/bucket` package is a TypeScript command-line utility that handles bidirectional synchronization (`push` and `pull`) for `workspace-assets` and `workspace-creative`.
...
The self-hosted Penpot editor uses the `workspace-penpot` bucket to store layout files, custom assets, images, and user avatars uploaded directly inside the editor UI.
