...
The `%PROJECT_DOMAIN%` project runs a series of self-hosted layered services configured in `studio/design/infrastructure/docker/compose.yaml` to ensure absolute data sovereignty, collaboration, and automated design-to-code pipelines.

---

## 1. Penpot Design System & Aide Assistant

%PROJECT_DOMAIN% utilizes a distributed Penpot stack integrated with Aide, an AI-native engineering assistant.
...
- **SVG Optimization:** Ensure exported SVG code is optimized, follows the project's icon standards, and is integrated via the `@repo/studio/assets/icons` registry.
