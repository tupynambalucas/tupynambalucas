...
│   │   ├── github-renderer.pipeline.ts # Standard renderer README compiler target
...
- **Extensible Registry**: Independent compilation targets (e.g., GitHub renderer READMEs, handbook manuals, or design systems specs) are registered inside `src/pipelines/index.ts`.
...
- `pnpm --filter @/renderer generate`: Runs the active document compilation pipelines locally, writing previews to `generated/`.
- `pnpm --filter @/renderer clean`: Removes all compiled files, caches, and output build directories.
- `pnpm --filter @/renderer typecheck`: Executes TypeScript compiler type validation.
- `pnpm --filter @/renderer lint`: Performs static ESLint checks.
