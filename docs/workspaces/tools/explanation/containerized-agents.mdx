...
The agent containers run inside an isolated network namespace, communicating securely with the unified **AgentGateway** (running in the `/platform` workspace) via bridge network routing. No host-level port exposure is required for inter-service communication.

```mermaid
graph TD
    subgraph host [Host Machine]
        direction TB
        A[Developer Shell / VS Code Tasks]
    end
...
Inside the agent containers, MCP services are resolved through the central Go-based **AgentGateway** (aliased and bridged on the virtual `tupynambalucas-platform-net` network). This decouples the agent containers from individual tool configurations.
...
The agent, platform, and MCP stacks are decoupled. Either stack can be started or stopped independently. The agents will automatically connect to the platform's `AgentGateway` once it is up.
:::
