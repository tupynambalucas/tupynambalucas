...
For complete technical specifications on edge TLS termination, single-level wildcard domain schemes (`*-dev.%PROJECT_DOMAIN%`), and reverse proxy topologies, refer to the [Infrastructure Reference](/workspaces/infrastructure/reference/architecture#2-reverse-proxy--request-routing).
:::
...
3. Select **Cloudflared** as the connector type and name the tunnel `workspace-minikube`.
...
2. Assign the public domain (e.g., `grafana.development.%PROJECT_DOMAIN%`).
3. Add an **Allow** policy restricting access to authorized developer email addresses via **One-Time PIN (OTP)**.

### AI Agent & MCP Ingress (`agentgateway-mcp-dev.%PROJECT_DOMAIN%`)
...
1. For `agentgateway-mcp-dev.%PROJECT_DOMAIN%`, configure an Access Policy with **Action: Bypass** or assign a **Service Token** and set the tunnel path to `/mcp`.
