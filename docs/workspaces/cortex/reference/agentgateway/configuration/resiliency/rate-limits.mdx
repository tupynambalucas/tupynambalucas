...
title: Rate limiting
description: >-
  Enforce budget and spend limits per key by controlling request and token
  usage.
sidebar_position: 2
...
In additional to simple request-based rate limits, agentgateway can limit requests based on _tokens_ for [LLM consumption](https://agentgateway.dev/docs/standalone/latest).
...
To enable token-based rate limiting, set the rate limiting type to `tokens`. This example shows only the `localRateLimit` policy; attach it to a route as shown in the complete examples in the [Configuration](/workspaces/cortex/reference/agentgateway/configuration/resiliency/rate-limits#configuration) section.
...
Agentgateway connects to any server that implements the [Envoy Rate Limit gRPC service](https://www.envoyproxy.io/docs/envoy/latest/api-v3/service/ratelimit/v3/rls.proto). The Envoy project provides a reference [`ratelimit`](https://github.com/envoyproxy/ratelimit) server that stores counters in Redis. If you already run a compatible service, skip to [Connect agentgateway to the rate limit server](/workspaces/cortex/reference/agentgateway/configuration/resiliency/rate-limits#connect-agentgateway-to-the-rate-limit-server) and point `host` at it instead.
...
With the server running, configure agentgateway to connect to it and specify which descriptors to send. Each descriptor value is a [CEL expression](/workspaces/cortex/reference/agentgateway/configuration/traffic-management/transformations).
...
Each descriptor value is a [CEL expression](/workspaces/cortex/reference/agentgateway/configuration/traffic-management/transformations).
...
To apply different rate limits based on the request, use the `conditional` field. For example, you can apply stricter limits on writes than on reads. For details, see [Conditional policies](/workspaces/cortex/reference/agentgateway/configuration/policies/conditional-policies).
