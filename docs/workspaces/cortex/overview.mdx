...
1. **Ingress Gateway Plane (`/gateway`)**: Exposes the visual playground and API endpoints for Downstream clients. Network traffic is routed via Traefik to the AgentGateway, which then federates requests to the MCP pods.
...
3. **Data Plane (`/mcp`)**: Houses individual Model Context Protocol adapters (GitHub, Firecrawl, Grafana, Playwright, Memory, Kubectl) that stream tools to active LLM contexts.
