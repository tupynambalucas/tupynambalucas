...
3. **ExtMCP Guardrails**: AgentGateway delegates every `tools/call` and
   `tools/list` request to the `mcp-guardrails` gRPC sidecar, which:
   - Rewrites `localhost` / `127.0.0.1` URLs to `host.docker.internal`.
   - Injects `system_instruction` metadata into tool listings.
