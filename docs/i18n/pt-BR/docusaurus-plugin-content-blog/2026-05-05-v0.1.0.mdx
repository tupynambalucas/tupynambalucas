...
O lançamento da versão fundacional do tupynambalucas.dev (v0.1.0) estabelece a arquitetura core do monorepo, as fronteiras de isolamento de workspaces e os alicerces de domínio para a loja da comunidade local e para o hub da plataforma global.
...
- **Arquitetura de Backend em Camadas**: Implementação da API Fastify v5 (`@tupynambalucas-hub/api`) utilizando a arquitetura em camadas estritas (`Controller -> Service -> Repository -> Model`).
...
- **Workspace de Frontend**: Criação do `@tupynambalucas-hub/web` usando React 19, apresentando interfaces estilizadas com CSS Modules, roteamento de views de cliente e integração com a API do backend. Inclui módulos de autenticação, painel do usuário, checkout da loja e administração local.

### Workspace Portal (Hub SaaS Global)

- **Infraestrutura Base**: Estruturação do esqueleto do workspace do portal (`@tupynambalucas-hub/api`, `@tupynambalucas-hub/web` e `@tupynambalucas-hub/core`).
...
- **Design Tokens**: Padronização de cores, espaçamentos, tipografia e ativos visuais no pacote `@tupynambalucas-studio/design` para exportar um tema unificado.
