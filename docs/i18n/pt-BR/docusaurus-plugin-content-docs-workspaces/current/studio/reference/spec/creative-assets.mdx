...
O pacote `@tupynambalucas-studio/assets` serve como o repositório central para todos os designs da marca, arquivos mestre de projetos brutos e ativos compilados prontos para produção.
...
- **Em Produção (Cópia em Tempo de Build)**: O sistema de build da aplicação usa o `vite-plugin-static-copy` para replicar automaticamente os arquivos de `node_modules/@tupynambalucas-studio/assets/assets/brand/` para o diretório de saída de build `dist/` em tempo de empacotamento.
