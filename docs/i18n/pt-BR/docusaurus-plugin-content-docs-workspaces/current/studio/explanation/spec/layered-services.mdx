...
O projeto `%PROJECT_DOMAIN%` executa uma série de serviços em camadas self-hosted configurados em `studio/design/infrastructure/docker/compose.yaml` para garantir soberania absoluta de dados, colaboração e pipelines automatizados de design para código.

---

## 1. Sistema de Design Penpot & Assistente Aide

O %PROJECT_DOMAIN% utiliza uma stack do Penpot distribuída integrada com o Aide, um assistente de engenharia AI-native.
...
- **Otimização de SVG:** Certifique-se de que o código SVG exportado está otimizado, segue os padrões de ícones do projeto e está integrado através do registro `@repo/studio/assets/icons`.
