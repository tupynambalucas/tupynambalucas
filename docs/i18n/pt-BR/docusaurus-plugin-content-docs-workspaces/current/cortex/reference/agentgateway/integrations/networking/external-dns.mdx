...
spec:
  ...
  - name: https
    protocol: HTTPS
    port: 443
    hostname: "ai.%PROJECT_DOMAIN%"  # External DNS creates this record
```
...
metadata:
  name: ai-api
  annotations:
    external-dns.alpha.kubernetes.io/hostname: api.%PROJECT_DOMAIN%
spec:
  parentRefs:
  - name: agentgateway
  hostnames:
...
metadata:
  annotations:
    ...
    external-dns.alpha.kubernetes.io/hostname: ai.%PROJECT_DOMAIN%
    # Set TTL
