...
Request: /anything/team1/foo
   ...
   Child Route "child-foo" (matches /anything/team1/foo)
         |
         v
   Backend: team1-foo.%PROJECT_DOMAIN%:8080
```
...
1. Create the configuration file.
   ...
   - name: team1-routes
     ...
     - name: child-foo
       ...
       backends:
       - host: team1-foo.%PROJECT_DOMAIN%:8080
     - name: child-bar
       matches:
       - path:
           pathPrefix: /anything/team1/bar
       backends:
       - host: team1-bar.%PROJECT_DOMAIN%:8080
   EOF
   ```
...
1. Create the configuration file.
   ...
   - name: api-routes
     ...
     - name: child-users
       ...
       backends:
       - host: users-service.%PROJECT_DOMAIN%:8080
     - name: child-orders
       matches:
       - path:
   ...
   - name: orders-routes
     ...
     - name: grandchild-list
       ...
       backends:
       - host: orders-list.%PROJECT_DOMAIN%:8080
     - name: grandchild-detail
       matches:
       - path:
           pathPrefix: /api/orders/detail
       backends:
       - host: orders-detail.%PROJECT_DOMAIN%:8080
   EOF
   ```
