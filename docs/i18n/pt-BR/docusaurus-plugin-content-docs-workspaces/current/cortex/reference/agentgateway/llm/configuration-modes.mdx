...
description: 'Use traditional HTTP routing configuration for advanced use cases like path-based routing and'
sidebar_position: 2
---
...
Both modes serve traffic on a [gateway](/workspaces/cortex/reference/agentgateway/configuration/gateways), which
...
llm:
  policies:
    jwtAuth:
      ...
      audiences: [api.%PROJECT_DOMAIN%]
      jwks:
        file: ./public-key.json
    authorization:
      rules:
      - 'jwt.email.endsWith("@%PROJECT_DOMAIN%")'
  models:
  - name: claude-haiku
    provider: anthropic
    params:
...
For more MCP listener context, see [MCP overview](/workspaces/cortex/reference/agentgateway/mcp).
