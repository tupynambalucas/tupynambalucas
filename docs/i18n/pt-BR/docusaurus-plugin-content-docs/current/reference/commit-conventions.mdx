...
Para manter a qualidade do código, a consistência de estilo e a segurança de tipos no monorepo **%PROJECT_DOMAIN%**, usamos um pipeline automatizado que verifica todas as alterações antes que elas sejam commitadas localmente e antes de serem mescladas na nuvem.
...
3. Ele mapeia os arquivos alterados para seus respectivos workspaces do monorepo:
   - Arquivos em `hub/` $\rightarrow$ verificação de tipos dos pacotes `@repo/hub/*`.
   - Arquivos em `cortex/` $\rightarrow$ verificação de tipos de `@/cortex`.
   - Arquivos em `studio/` $\rightarrow$ verificação de tipos de `@repo/studio/assets`.
   - Arquivos em `tools/` $\rightarrow$ verificação de tipos de `@repo/tools/*`.
   - Arquivos em `docs/` $\rightarrow$ verificação de tipos de `@/docs`.
...
5. Ele executa a verificação de tipos direcionada em paralelo usando filtros do Turborepo (ex.: `npx turbo run typecheck --filter=@repo/hub/*`).
...
As regras são declaradas no [package.json](file:///D:/projects/workspace/package.json) da raiz:
...
A configuração do fluxo de trabalho é definida em [.github/workflows/ci.yaml](file:///D:/projects/workspace/.github/workflows/ci.yaml):

- Instala dependências usando `pnpm install --frozen-lockfile`.
- Executa a suíte completa de verificação de tipos: `pnpm typecheck --filter=!@repo/hub/*`.
- Valida o estilo e a qualidade do código: `pnpm lint --filter=!@repo/hub/*`.
