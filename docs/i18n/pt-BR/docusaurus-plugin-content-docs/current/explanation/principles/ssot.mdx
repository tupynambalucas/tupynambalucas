...
description: Desacoplamento e governança centralizada de esquemas e modelos compartilhados no monorepo %PROJECT_DOMAIN%.
---
...
No %PROJECT_DOMAIN%, o princípio SSOT é implementado nativamente por meio de nossa arquitetura monorepo:

### A. Bibliotecas Core Centralizadas

Os workspaces `@repo/hub/core` e `@repo/hub/core` servem como o SSOT absoluto para seus respectivos contextos delimitados. Qualquer esquema, interface ou tipo compartilhado por mais de uma aplicação (como a API do backend e a SPA web do frontend) deve ser declarado aqui primeiro, seguindo nossa diretriz de **Design Direcionado ao Core (Core First Design)**.

### B. Compartilhamento de Contratos e Tipos

Aplicações downstream como `@repo/hub/api` (Fastify) e `@repo/hub/web` (React SPA) não mantêm interfaces de objetos de transferência de dados (DTO) ou modelos de banco de dados duplicados. Em vez disso, importam diretamente tipos, interfaces e classes do pacote core compartilhado:
