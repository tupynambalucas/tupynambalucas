...
description: Combinando contextos delimitados do Domain-Driven Design (DDD) e a modularidade do Feature-Sliced Design (FSD) no frontend do projeto %PROJECT_DOMAIN%.
---
...
No %PROJECT_DOMAIN%, a combinação de DDD e FSD governa tanto a estrutura de pastas quanto as regras de importação:

### A. Segregação de Contexto Delimitado

O monorepo é dividido em contextos distintos (como `hub/`, `cortex/`, `platform/`) no nível raiz. Esses limites garantem que os portais de desenvolvedores (Hub) e os serviços globais do sistema e IA (Cortex/Platform) permaneçam estrutural e logicamente independentes. Tokens de design compartilhados ou ativos de marca são importados exclusivamente do workspace `@repo/studio/assets`.
