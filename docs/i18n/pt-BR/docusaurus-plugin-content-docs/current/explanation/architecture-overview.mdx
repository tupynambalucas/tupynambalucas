---
title: Visão Geral
description: Visão geral abrangente da estrutura monorepo, contextos delimitados e pilha de tecnologia do %PROJECT_DOMAIN%.
sidebar_position: 1
---
...
%PROJECT_DOMAIN% é um portal de desenvolvedor pessoal, portfólio, blog e motor de automação. O sistema é construído sobre uma arquitetura **Monorepo** usando **PNPM Workspaces** e **Turborepo**, priorizando alto desempenho, tipagem estrita e isolamento de domínio por meio de uma estratégia de **Contexto Delimitado** (Bounded Context).

A arquitetura é projetada para hospedar o site central do desenvolvedor (`hub`) e o compilador automatizado de estatísticas (`renderer`), logicamente desacoplados no nível raiz.

---

## 2. Estrutura Monorepo e Contextos Delimitados

A base de código do %PROJECT_DOMAIN% é organizada em **Contextos de Domínio** no nível raiz. Para um detalhamento dos papéis da aplicação, filosofia de contextos e estruturas de diretórios, por favor consulte o guia de Contextos Delimitados:
...
- **Acoplamento de Infraestrutura:** Orquestração de serviços de suporte como pré-requisitos para o desenvolvimento da aplicação (por exemplo, `pnpm hub:up` ou `pnpm cortex:up` inicia bancos de dados/serviços de suporte antes da API).
...
Para garantir manutenibilidade extrema, legibilidade a longo prazo e limites claros entre nossos contextos de domínio, o %PROJECT_DOMAIN% segue quatro princípios arquiteturais fundamentais.
...
import type { IUser } from '@repo/hub/core';
...
Projetado para **Excelência Auto-Hospedada**, nossa orquestração nativa Kubernetes, pipelines de desenvolvimento Skaffold, estratégia de variáveis de ambiente e padrões de rede são gerenciados centralmente no workspace Infrastructure.

Para um guia completo e aprofundado sobre estruturas de diretórios, implantação no Minikube, roteamento com Cloudflare tunnel e overlays do Kustomize, explore a documentação dedicada do workspace Infrastructure:

<DocCardList
  items={[
    {
      type: 'link',
      href: '/workspaces/infrastructure/intro',
      label: 'Workspace Infrastructure',
      description:
        'Explore a orquestração centralizada do Kubernetes, pipelines de implantação GitOps e topologia de rede edge-to-cluster.',
    },
    {
      type: 'link',
      href: '/workspaces/infrastructure/guides/local-development',
      label: 'Configuração de Desenvolvimento Local',
      description:
        'Aprenda como executar o monorepo localmente usando Minikube e Skaffold para recarregamento rápido (hot-reloading).',
    },
  ]}
/>
...
O %PROJECT_DOMAIN% segue uma estratégia de segurança em múltiplas camadas. Para detalhes detalhados de implementação, consulte o documento de **[Arquitetura de Segurança](/docs/reference/security)**.
