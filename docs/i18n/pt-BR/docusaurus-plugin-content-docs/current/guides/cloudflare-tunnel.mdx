...
Para especificações técnicas completas sobre terminação TLS na edge, esquemas de domínio wildcard de nível único (`*-dev.tupynambalucas.dev`) e topologias de proxy reverso, consulte a [Referência de Infraestrutura](/docs/reference/infrastructure#4-proxy-reverso--roteamento-de-requisições).
:::
...
| `memory-dev`           | `tupynambalucas.dev` | `HTTP`          | `traefik.platform.svc.cluster.local:80` | `/`     |
| `memory-api-dev`       | `tupynambalucas.dev` | `HTTP`          | `traefik.platform.svc.cluster.local:80` | `/`     |
