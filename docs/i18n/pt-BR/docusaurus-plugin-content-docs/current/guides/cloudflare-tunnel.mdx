...
O tráfego de entrada público é roteado através da rede edge global da Cloudflare diretamente para o cluster por meio de um túnel QUIC criptografado.

:::info[Referência Técnica de Arquitetura]
Para especificações técnicas completas sobre terminação TLS na edge, esquemas de domínio wildcard de nível único (`*-dev.%PROJECT_DOMAIN%`) e topologias de proxy reverso, consulte a [Referência de Infraestrutura](/docs/reference/infrastructure#4-proxy-reverso--roteamento-de-requisições).
:::
...
3. Selecione **Cloudflared** como tipo de conector e nomeie o túnel como `workspace-minikube`.
...
2. Atribua o domínio público (ex: `grafana.development.%PROJECT_DOMAIN%`).
...
### Ingress de Agentes de IA & MCP (`agentgateway-mcp-dev.%PROJECT_DOMAIN%`)
...
1. Para `agentgateway-mcp-dev.%PROJECT_DOMAIN%`, configure uma Política de Acesso com **Action: Bypass** ou atribua um **Service Token** e defina o caminho da rota do túnel para `/mcp`.
