...
3. It maps the changed files to their corresponding monorepo workspaces:
   - Files in `instance/` $\rightarrow$ typecheck `@tupynambalucas-hub/*` packages.
   - Files in `studio/` $\rightarrow$ typecheck `@tupynambalucas-studio/assets`.
   - Files in `tools/` $\rightarrow$ typecheck `@tupynambalucas/tools`.
   - Files in `docs/` $\rightarrow$ typecheck `@tupynambalucas/docs`.
   - Files in `portal/` $\rightarrow$ typecheck `@tupynambalucas-hub/*`.
