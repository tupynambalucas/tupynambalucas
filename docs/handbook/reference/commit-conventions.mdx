...
To maintain code quality, styling consistency, and type safety across the **%PROJECT_DOMAIN%** monorepo, we use an automated pipeline that checks all changes before they are committed locally and before they are merged in the cloud.
...
Running a full typecheck across all monorepo packages (`pnpm typecheck`) can take several seconds. To keep the commit process fast, we use a custom script located at [precommit-typecheck.ts](file:///D:/projects/workspace/tools/scripts/precommit-typecheck.ts).
...
3. It maps the changed files to their corresponding monorepo workspaces:
   - Files in `hub/` $\rightarrow$ typecheck `@repo/hub/*` packages.
   - Files in `cortex/` $\rightarrow$ typecheck `@/cortex`.
   - Files in `studio/` $\rightarrow$ typecheck `@repo/studio/assets`.
   - Files in `tools/` $\rightarrow$ typecheck `@repo/tools/*`.
   - Files in `docs/` $\rightarrow$ typecheck `@/docs`.
...
5. It executes the targeted typecheck in parallel using Turborepo filters (e.g., `npx turbo run typecheck --filter=@repo/hub/*`).
...
The rules are declared in the root [package.json](file:///D:/projects/workspace/package.json):
...
The workflow configuration is defined in [.github/workflows/ci.yaml](file:///D:/projects/workspace/.github/workflows/ci.yaml):

- It installs dependencies using `pnpm install --frozen-lockfile`.
- It executes the full typecheck suite: `pnpm typecheck --filter=!@repo/hub/*`.
- It validates code style and quality: `pnpm lint --filter=!@repo/hub/*`.
