...
This document defines the strict coding conventions and quality requirements for the **%PROJECT_DOMAIN%** project. All code must adhere to these standards to ensure consistency and maintainability across the monorepo.
...
### 4.1. Core Packages (`@repo/hub/core` / `@repo/hub/core`)
...
function AdvancedCycleManager() {
  ...
  const [products, setProducts] = React.useState([
    { id: 'uuid-1', name: 'Green Lettuce', price: 4.5, isValid: true },
    { id: 'uuid-2', name: 'Cherry Tomato', price: 0, isValid: false },
  ]);
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <h3 className={styles.title}>Cycle Manager</h3>
        ...
        >
          {hasErrors === true ? '⚠ INVALID ITEMS' : '✓ READY TO SAVE'}
        </span>
      </div>
      ...
      >
        {isSubmitting === true ? 'Processing Update...' : 'Save Cycle Changes'}
      </button>
