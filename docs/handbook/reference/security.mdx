---
title: Security
description: Comprehensive overview of security measures, bot protection, and brute-force mitigation in tupynambalucas.dev.
sidebar_position: 4
---

# Security Architecture

tupynambalucas.dev implements a multi-layered security strategy to protect community data and ensure the integrity of sharing cycles. Our approach combines industry-standard protocols with specialized bot protection and brute-force mitigation.
...
- **Mandatory DTOs**: The `LoginDTOSchema` and `RegisterDTOSchema` in `@tupynambalucas-hub/core` strictly require the `turnstileToken`.
...
The following security keys are mandatory for the **Hub** stack to operate. Each variable lives in the env file owned by the responsible application, following the project's **Bounded Context** isolation principle.
