...
In this tutorial, we will walk through the exact process of contributing code to the %PROJECT_DOMAIN% repository. We will create a feature branch, make a small change, run our automated validations, and create a Pull Request.
...
You will be prompted to select the package that changed. Use the arrow keys to find `@/docs`, press `Space` to select it, and press `Enter`.
