...
A Bounded Context defines the explicit boundary inside which a domain model is valid. Inside this boundary, our Ubiquitous Language has precise and indisputable meanings. Within each bounded context in `%PROJECT_DOMAIN%`:
...
The **Shared Kernel** pattern is utilized when contexts share a stable portion of domain schemas or variables. In our system, **`@repo/studio/assets`** acts as a Shared Kernel. Both `@repo/hub/web` and `@/renderer` import design tokens and SVG icons directly from this package.
...
- **Horizontal Layering**: Internal architectural layers. For instance, the `@repo/hub/web` (UI/Feature layer) is completely decoupled from `@repo/hub/core` (Domain model layer), forcing the dependency flow downward.
...
When a Bounded Context includes containerized deployment units (managed locally via Podman Compose or Skaffold/Minikube Kubernetes manifests), the directory layout must be structured to match its architectural alignment:
