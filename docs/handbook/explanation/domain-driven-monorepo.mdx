...
| **Platform (`platform/`)** | **Generic Domain**       | Go-based AgentGateway proxy, OpenTelemetry monitoring collector, and Turborepo cache servers.               |
...
├── platform/                 # Platform Context (Gateway, OTel, Cache)
├── tools/                    # Tools Context (AI Agents & MCP Servers)
...
Our **`AgentGateway`** (under `/platform`) acts as a network-level ACL. Downstream AI agent containers in `tools/` execute complex, non-standard MCP commands. Instead of coupling the agents to raw endpoints, they communicate through the clean, federated `AgentGateway` Go-ingress proxy, which translates and routes payloads seamlessly.
