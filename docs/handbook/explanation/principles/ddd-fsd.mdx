...
The monorepo is split into distinct contexts (`instance/` and `portal/`) at the root level. These boundaries ensure that community operations (Instance) and global SaaS onboarding workflows (Portal) remain structurally and logically independent. Shared design tokens or brand assets are imported exclusively from the `@tupynambalucas-studio/assets` workspace.
