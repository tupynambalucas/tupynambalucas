...
description: Leveraging Domain-Driven Design (DDD) Bounded Contexts and Feature-Sliced Design (FSD) in %PROJECT_DOMAIN% frontends.
---
...
In %PROJECT_DOMAIN%, the combination of DDD and FSD governs both the folder structure and the import rules:

### A. Bounded Context Segregation

The monorepo is split into distinct contexts (such as `hub/`, `cortex/`, `platform/`) at the root level. These boundaries ensure that developer hub portals (Hub) and global system platform and AI services (Cortex/Platform) remain structurally and logically independent. Shared design tokens or brand assets are imported exclusively from the `@repo/studio/assets` workspace.
