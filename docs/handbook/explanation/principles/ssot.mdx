...
description: Decoupling and centralized governance of shared schemas and models in the %PROJECT_DOMAIN% monorepo.
---
...
In %PROJECT_DOMAIN%, the SSOT principle is implemented natively via our monorepo architecture:

### A. Centralized Core Libraries

The `@repo/hub/core` and `@repo/hub/core` workspaces serve as the absolute SSOT for their respective bounded contexts. Any schema, interface, or type shared by more than one application (such as backend API and frontend web SPA) must be declared here first, following our **Core First Design** guardrail.

### B. Contract & Type Sharing

Downstream applications like `@repo/hub/api` (Fastify) and `@repo/hub/web` (React SPA) do not maintain duplicate data transfer object (DTO) interfaces or database models. Instead, they directly import types, interfaces, and classes from the shared core package:

```typescript
// Core package definition
// hub/packages/core/src/domains/cycle/cycle.types.ts
...
// hub/services/api/src/domains/cycle/cycle.service.ts
import type { ICycle } from '@repo/hub/core';
```
